...
load_dotenv()
URL_LOGIN = 'https://visa.vfsglobal.com/blr/ru/ltu/login'
login = os.getenv('LOGIN')
password = os.getenv('PASSWORD')
data = {
    'center_name': 'Lithuania Visa Application Center- Minsk',
    'category_name': 'Short-Term VISA Type C',
    'subcategory_name': 'Short term max 90 days',
    # 'delay_min': 10
}


async def login_lt(delay_sec=10):
    ...
    try:
        ...
        if button.get_attribute('disabled') == 'true':
            mess = f'[LT] Дат нету, следующая попытка через {delay} секунд - {datetime.now().strftime("%H:%M:%S")}'
            print(mess)
            for id in ids:
            ...
            button.click()
            mess = '[LT] ЕСТЬ ДАТА, БЕГОМ К КОМПУ!!!'
            for id in ids:
                await bot.send_message(id, mess)

    except Exception:
        mess = '[LT] IP забанен, перезагрузи модем'
        print(mess)
        for id in ids:
            await bot.send_message(id, mess)
        driver.quit()
